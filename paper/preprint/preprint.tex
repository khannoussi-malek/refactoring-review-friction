...
Across the same twelve projects, the \textbf{commit-side rate runs 82.6--98.3\%} while the \textbf{ticket realisation rate runs 12.0--69.0\%, with none above 69.0\%} (Definition 1 and Definition 2, \S\kern0.13em 3.1; Table 1). The headline case:
...
\caption{Corpus eligibility across the 38 probed projects, both traceability channels, with commits per ticket, the arithmetic ceiling of Section 3.1.4 and the share of it reached (continued).}\\
...
\textbf{The ceiling, and why it reframes the divergence.} A commit that cites a ticket contributes at most one NEW distinct ticket to the ticket-side numerator, so the rate cannot exceed \texttt{commit-\allowbreak{}side x commits /\allowbreak{} tickets} (method \S\kern0.13em 3.1.4). That bound BINDS for all twelve: it runs 13.7\% (kylin) to 81.6\% (ranger), and every project reaches 80--95\% of it. The ticket-side spread is therefore almost entirely a spread in commits per ticket, not in citation discipline --- \texttt{TRR/\allowbreak{}ceiling} has a median of 0.89 and a range of only 1.19x across the twelve, against a 5.8x spread in the rate itself.
