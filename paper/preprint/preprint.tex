...
\begin{quote}
\textbf{[DETERMINATION PENDING]} Whether the ticket realisation rate of \S\kern0.13em \ref{sec:3.1} is the same quantity Bachmann et al. measured, or a distinct one, is \textbf{not settled in this draft}. The argument for distinctness is that \S\kern0.13em \ref{sec:3.1.1} admits every issue type and every status and conditions on nothing, whereas Bachmann conditions on bugs and bug-fix commits. That argument has not been adjudicated against the paper's own text, and the naming claim in \S\kern0.13em \ref{sec:3.1} stands or falls with it. See \texttt{paper/\allowbreak{}REVISION\_\allowbreak{}LOG.\allowbreak{}md}, GATE.

\end{quote}

\textbf{Bird et al. (ESEC/FSE'09), \emph{Fair and Balanced? Bias in Bug-Fix Datasets}} \cite{bird2009fair} --- C. Bird, A. Bachmann, E. Aune, J. Duffy, A. Bernstein, V. Filkov and P. Devanbu --- is the reason any of this matters. Missing links are not missing at random, so a dataset built from linked records is a biased sample of the work, and models fitted to it inherit the bias. \textbf{This is the same argument our \S\kern0.13em \ref{sec:7.3} makes for architectural change}, arrived at independently and seventeen years later, and we cite it as the prior statement of the principle rather than as a parallel.
...
\texttt{fill} is the share of the attainable maximum actually reached --- how efficiently a project's citing commits spread across distinct tickets rather than piling onto a few. \textbf{\texttt{fill} is the quantity a claim about citation discipline needs}; \texttt{ceiling} is a property of how much code the project writes per ticket it files. Tables~\ref{tab:eligibility} and~\ref{tab:ticketside} report both. \S\kern0.13em \ref{sec:4.2} shows that in this corpus almost all of the ticket-side variation is ceiling and almost none of it is fill.
...
\textbf{Hadoop itself is not one of the 38.} It is the corpus the exploratory work was done on, and it is used here only as the worked example for multi-key matching. Every rate in Tables~\ref{tab:eligibility} and~\ref{tab:ticketside} is from a project Hadoop is not.
...
\textbf{Ticket-side coverage counts keys that resolve in the tracker.} Sqoop cites 790 distinct keys of which 122 have no tracker record, so its ticket-side rate rests on the 668 that resolve (\S\kern0.13em \ref{sec:5.3}). This is mode 6 inside a passing project, and it means ticket-side rates are, strictly, coverage of \emph{resolvable} tickets.
...
No funding was received. The author declares no competing interests. The work was carried out independently, alongside full-time employment, which is stated in Section~\ref{sec:6.8} because it explains the shape of the study's limitations.
