...
\item \textbf{The population finding.} 12 of 38 pass and no project outside one ecosystem clears the bar. SEOSS's own table contains the ingredients --- Apache at the top, JBoss at the bottom --- but does not draw the inference. The ecosystem label is a hand classification and \S\kern0.13em 4.1 reports the measured variables that failed to replace it.
...
The commit-side/ticket-side asymmetry this paper measures was studied on the bug-side a decade and a half ago, and that literature is the direct ancestor of \S\kern0.13em 4.2. It is set out here rather than merely listed, because two of its findings bound what this paper can claim.
...
\begin{quote}
\textbf{[DETERMINATION PENDING]} Whether the ticket realisation rate of \S\kern0.13em 3.1 is the same quantity Bachmann et al. measured, or a distinct one, is \textbf{not settled in this draft}. The argument for distinctness is that \S\kern0.13em 3.1.1 admits every issue type and every status and conditions on nothing, whereas Bachmann conditions on bugs and bug-fix commits. That argument has not been adjudicated against the paper's own text, and the naming claim in \S\kern0.13em 3.1 stands or falls with it. See \texttt{paper/\allowbreak{}REVISION\_\allowbreak{}LOG.\allowbreak{}md}, GATE.

\end{quote}

\textbf{Bird et al. (ESEC/FSE'09), \emph{Fair and Balanced? Bias in Bug-Fix Datasets}} --- C. Bird, A. Bachmann, E. Aune, J. Duffy, A. Bernstein, V. Filkov and P. Devanbu --- is the reason any of this matters. Missing links are not missing at random, so a dataset built from linked records is a biased sample of the work, and models fitted to it inherit the bias. \textbf{This is the same argument our \S\kern0.13em 7.3 makes for architectural change}, arrived at independently and seventeen years later, and we cite it as the prior statement of the principle rather than as a parallel.
...
\textbf{Herzig, Just and Zeller (ICSE'13), \emph{It's not a bug, it's a feature: how misclassification impacts bug prediction}} bears directly on \S\kern0.13em 3.1.1's decision to admit all issue types. In a manual examination of \textbf{more than 7,000 issue reports across five open-source projects they found 33.8\% misclassified} --- filed as bugs but resolving to a feature, a documentation update or an internal refactoring --- and \textbf{39\% of files marked defective never had a bug}. Our denominator admits every type precisely so that no misclassification can move a ticket in or out of it. That immunises the ticket realisation rate against the Herzig effect, and it is also why our rate is \emph{not} comparable to any rate computed on a resolution-filtered or type-filtered population, including Vieira et al.'s.

\textbf{Automated link recovery is the standing partial answer to modes 4--6.} ReLink (Wu, Zhang, Kim and Cheung, ESEC/FSE'11) learns the features of explicit links --- time proximity, author identity, textual similarity between the bug report and the change --- and recovers missing ones at accuracy well above the traditional regex heuristics, and a substantial literature has followed it. \S\kern0.13em 5.5 and \S\kern0.13em 7.2 argue that a project's key set cannot be recovered from its tracker, which remains true: ReLink and its successors recover \emph{links}, not \emph{key sets}, and they operate on a project already known to be a candidate. But the unqualified claim that missing links are unrecoverable would overstate the gap, and \S\kern0.13em 5.5 is qualified accordingly.
...
The literature reports one number and issue-anchored studies depend on another. Both are stated here as definitions so that the divergence in \S\kern0.13em 4 is a comparison between named quantities rather than between two informal readings of the word ``traceability''.
...
\textbf{$\mathrm{TRR}_\mathrm{frozen}$} & the Public Jira Dataset (Zenodo 15719919), a snapshot predating every pinned sha & the same pinned shas & all 38 probed projects \\
\end{longtable}
\addtocounter{table}{-1}
\endgroup
...
\toprule
\textbf{\{ k $\in$ Tickets(p, T) : k realised \}} & \textbf{$\leq$ CSR(p) $\cdot$} & \textbf{$C_p$} \\
\midrule
\endfirsthead
\toprule
\textbf{\{ k $\in$ Tickets(p, T) : k realised \}} & \textbf{$\leq$ CSR(p) $\cdot$} & \textbf{$C_p$} \\
...
\texttt{fill} is the share of the attainable maximum actually reached --- how efficiently a project's citing commits spread across distinct tickets rather than piling onto a few. \textbf{\texttt{fill} is the quantity a claim about citation discipline needs}; \texttt{ceiling} is a property of how much code the project writes per ticket it files. Tables 1 and 3 report both. \S\kern0.13em 4.2 shows that in this corpus almost all of the ticket-side variation is ceiling and almost none of it is fill.

\textbf{The subscript propagates, and it matters.} \texttt{ceiling} and \texttt{fill} are both functions of TRR and of the ticket denominator, so each inherits whichever instantiation of \S\kern0.13em 3.1.3 produced it: $\mathrm{ceiling}_\mathrm{live}$ and $\mathrm{fill}_\mathrm{live}$ are computed against the live Jira read of 2026-07-25, $\mathrm{ceiling}_\mathrm{frozen}$ and $\mathrm{fill}_\mathrm{frozen}$ against the frozen snapshot. The two are not interchangeable and they do not always agree about whether the bound binds at all --- three projects have $\mathrm{ceiling}_\mathrm{frozen}$ at or above 100\% while every $\mathrm{ceiling}_\mathrm{live}$ is below it (\S\kern0.13em 4.2, Table 3). \textbf{Throughout this paper an unsubscripted \texttt{TRR}, \texttt{ceiling} or \texttt{fill} means the quantity in general rather than a measured value; every measured value carries its subscript}, in the prose, in the abstract and in the table captions. Where a project appears with two numbers for what looks like one quantity, the subscripts are the difference.
...
Across the same twelve projects, the \textbf{commit-side rate runs 82.6--98.3\%} while \textbf{$\mathrm{TRR}_\mathrm{live}$ runs 12.0--69.0\%, with none above 69.0\%} (Definition 1 and Definition 2, \S\kern0.13em 3.1; Table 1). The headline case:
...
\textbf{Most of that gap is not a discipline gap. It is the ceiling (\S\kern0.13em 3.1.4).} Hive has \textbf{0.61 commits per ticket}, so at 97.0\% commit-side its ticket-side rate cannot exceed \textbf{59.6\%} whatever anyone does --- and it reaches \textbf{0.94} of that. The pattern holds across all twelve:
...
\textbf{Every ceiling in this section is $\mathrm{ceiling}_\mathrm{live}$, and the frozen estimator does not agree that the bound binds.} Table 3 computes $\mathrm{ceiling}_\mathrm{frozen}$ against the frozen Jira snapshot and puts three of the twelve at or above 100\% --- Ozone \textbf{176.0\%}, Ranger \textbf{130.4\%}, Knox \textbf{100.0\%} --- where a bound above 100\% does not constrain anything. That is not a disagreement about the projects. It is what a 2026 commit window measured against an older tracker produces: the repository holds more citing commits than the snapshot holds tickets, so the arithmetic ceiling exceeds one and stops being a ceiling. The two estimators bound different denominators, and Table 3's own note says so. Neither figure is withdrawn.
...
Table 3 reports the same project at \textbf{$\mathrm{fill}_\mathrm{frozen}$ 0.00}, and the two are not in conflict. Under the frozen snapshot Kylin's $\mathrm{TRR}_\mathrm{frozen}$ is \textbf{0.04\%} --- two cited keys against a tracker of 4,989 --- because \textbf{99.72\% of its cited keys postdate the snapshot}, so the number cap removes almost the whole numerator. Its $\mathrm{ceiling}_\mathrm{frozen}$ is 16.3\%, well under the bound. The frozen estimator is registering the same truncated branch this section is about, from the other side: live, the window is too short for the tracker; frozen, the tracker is too old for the window. Kylin is the worst end-to-end case in the corpus at 11.91pp (Table 3), and it is flagged in both tables for that reason.
...
Reporting \S\kern0.13em 4.2 on the twelve that passed leaves the obvious objection open: the range 12.0--69.0\% is \emph{within-passing variation} in $\mathrm{TRR}_\mathrm{live}$, and says nothing about the 26 projects below the bar, whose ticket realisation rates were never measured. That objection is answered here by measuring all 38. \textbf{Table 3 (\texttt{paper/\allowbreak{}table3\_\allowbreak{}ticket\_\allowbreak{}side.\allowbreak{}md}) is the result}: one row per probed project, with the commit-side rate, commits per ticket, the \S\kern0.13em 3.1.4 ceiling, the ticket realisation rate, and the fill, plus a note column recording every reason a row should not be read at face value.

\textbf{Method.} The ticket realisation rate needs a tracker snapshot. Re-reading the live tracker for 26 more projects would have dated those denominators weeks after the twelve already measured, so the extended computation instead takes its denominators from the frozen public Jira corpus and aligns numerator and denominator in issue-number space (\S\kern0.13em 3.1.3). \textbf{The estimator is validated before it is used}: run against the live denominators for the twelve projects where the exact rate is already known, it reproduces that rate to a \textbf{mean absolute error of 0.45pp} across all twelve, with a \textbf{worst case of 2.14pp} (Kylin, where the tracker's issue numbering has enough gaps that number-capping undercounts). All 38 projects were then measured at their pinned shas.
...
\textbf{Quote 1.76pp / 11.91pp for anything in Table 3.} The favourable reading is also true and is the one to lead with: eleven of twelve land within 2.93pp, and the twelfth is Kylin, whose branch truncation (\S\kern0.13em 4.2.1) is the same defect showing up a second time.
...
\textbf{What the extension converts, and what it does not.} \S\kern0.13em 4.2's range was \emph{within-passing variation} and licensed no claim about the projects below the bar. It now licenses a bounded one: across the whole probe, a high commit-side rate does not predict a high ticket-side rate, and the bar selects for neither. \textbf{The caveat on Table 1 is not withdrawn} --- it remains true of the live measurement, and this is a different estimator against a different snapshot, reported alongside rather than merged into it.

Five projects have no usable rate and are excluded rather than scored zero, which is itself the taxonomy at work. Pinot (14 tracker issues), Dubbo (78) and RocketMQ (384) have denominators too small to carry a rate. ShardingSphere and SkyWalking have \textbf{no project record for their probed key at all} --- mode 1 and mode 6, a category rather than a low number (\S\kern0.13em 5.3).
...
A researcher sampling with GHS therefore gets stars, commits, contributors and license, and discovers post hoc that 26 of 38 candidates are unusable. That is what happened here, and \S\kern0.13em 5 is the taxonomy of the ways it happens.
...
4 & \textbf{Convention changed mid-\allowbreak{}history} & spring-\allowbreak{}batch: \texttt{BATCH-\allowbreak{}} in \textbf{45.6\%} of 7,035 commits overall, in \textbf{0 of the most recent 1,000} (back to 2021-06), and at exactly \textbf{0\% in every year since 2019} --- a single rate averages two regimes & full scan, \texttt{scripts/\allowbreak{}springbatch\_\allowbreak{}recency.\allowbreak{}py} \\
...
The probed key sets were derived empirically from commit messages (\S\kern0.13em 3.2), and the frozen public Jira corpus records every Apache project that existed at its snapshot. Intersecting the two turns mode 6 from an anecdote into a count: \textbf{\textbf{six} of the keys this study probes have no project record in that corpus at all}, and five of them are actually cited by commits.
...
\item \textbf{Keys with no tracker record at all.} Some probed keys have no project record in the frozen corpus. These are taxonomy modes 1 and 6, not low rates, and they are \textbf{excluded rather than scored as 0\%} --- scoring them would put a number where a category belongs, which is the error \S\kern0.13em 5.1 exists to prevent.
\end{enumerate}

\textbf{The truncation caveat on Table 1 is not withdrawn.} The 12.0--69.0\% range from the live measurement remains within-passing variation, computed on the twelve that had already cleared the bar. \S\kern0.13em 4.3 does not extend that measurement; it is a different estimator against a different snapshot, and it is reported alongside rather than merged into it.
...
\caption{Corpus eligibility across the 38 probed projects, both traceability channels, with commits per ticket, the arithmetic ceiling of Section 3.1.4 and the share of it reached. All quantities are the live measurement: $\mathrm{TRR}_\mathrm{live}$, $\mathrm{ceiling}_\mathrm{live}$, $\mathrm{fill}_\mathrm{live}$ (Section 3.1.3).}\label{tab:eligibility}\\
...
\textbf{Kylin's row is flagged, not featured.} At 83.9\% commit-side against 12.0\% ticket-side it is the sharpest divergence in the table, and earlier drafts led with it. \textbf{That example is withdrawn (\S\kern0.13em 4.2.1.)} The pinned commit reaches 968 commits beginning 2022-08-01, 7.5\% of the 12,937 on the repository's refs, so the ticket-side figure is a four-year commit window measured against a twelve-year tracker. The row is retained because the measurement is correct for the branch that was pinned; it should not be read as a fact about Kylin's traceability.



\textbf{The ceiling, and why it reframes the divergence.} A commit that cites a ticket contributes at most one NEW distinct ticket to the ticket-side numerator, so the rate cannot exceed \texttt{commit-\allowbreak{}side x commits /\allowbreak{} tickets} (method \S\kern0.13em 3.1.4). Every quantity in this table is the live measurement, so the bound here is $\mathrm{ceiling}_\mathrm{live}$, and it BINDS for all twelve: it runs 13.7\% (kylin) to 81.6\% (ranger), and every project reaches 80--95\% of it. The ticket-side spread is therefore almost entirely a spread in commits per ticket, not in citation discipline --- $\mathrm{fill}_\mathrm{live}$ has a median of 0.89 and a range of only 1.19x across the twelve, against a 5.8x spread in the rate itself. Table 3 computes $\mathrm{ceiling}_\mathrm{frozen}$ instead and puts three projects at or above 100\%, where the bound does not bind at all; the two are different denominators, not a disagreement (\S\kern0.13em 3.1.3, \S\kern0.13em 4.2).
...
Denominators come from the frozen public Jira corpus, not from a second live fetch; numerator and denominator are aligned in issue-number space (method \S\kern0.13em 3.1.3).
...
\textbf{Ceiling and fill.} A commit citing a ticket adds at most one NEW distinct ticket, so TRR $\leq$ CSR $\times$ commits / tickets (method \S\kern0.13em 3.1.4). \texttt{ceiling} is that bound; \texttt{TRR/\allowbreak{}ceiling} the share of it reached. A ceiling above 100\% does not bind --- it means the repository has more citing commits than the snapshot has tickets, which is what a 2026 commit window against an older tracker produces.
...
\caption{The ticket realisation rate across all 38 probed projects, with the note column recording every reason a row should not be read at face value. All quantities are the frozen measurement: $\mathrm{TRR}_\mathrm{frozen}$, $\mathrm{ceiling}_\mathrm{frozen}$, $\mathrm{fill}_\mathrm{frozen}$ (Section 3.1.3).}\label{tab:ticketside}\\
