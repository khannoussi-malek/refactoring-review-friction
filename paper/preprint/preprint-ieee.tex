...
\centering
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.4793\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.3365\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.1843\linewidth-2\tabcolsep\relax}}
\toprule
...
\texttt{fill} is the share of the attainable maximum actually reached --- how efficiently a project's citing commits spread across distinct tickets rather than piling onto a few. \textbf{\texttt{fill} is the quantity a claim about citation discipline needs}; \texttt{ceiling} is a property of how much code the project writes per ticket it files. Tables 1 and 3 report both. \S\kern0.13em \ref{sec:4.2} shows that in this corpus almost all of the ticket-side variation is ceiling and almost none of it is fill.
...
\textbf{Held-out discipline is mechanical.} Outcome data was never observed for seven projects --- Ozone, Tez, ZooKeeper, Ranger, Oozie, Knox, Sqoop. The rule is written down (\texttt{PROJECT\_\allowbreak{}STATE.\allowbreak{}md} \S\kern0.13em \ref{sec:3}); the ticket-side script requests one Jira field and aborts if it receives another; the matcher-validation script asks git for \texttt{\%H}, \texttt{\%s} and \texttt{\%b} and nothing else, so it \emph{cannot} compute a duration. HBase and Phoenix were dropped from the held-out corpus on 2026-07-30 and quarantined, because their committed artifacts are one subtraction away from per-ticket latency even though no outcome was ever observed for either; their coverage numbers remain usable and are reported.
...
\centering
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.4218\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.1711\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2359\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.1711\linewidth-2\tabcolsep\relax}}
\toprule
...
\centering
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.4048\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.1280\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.4672\linewidth-2\tabcolsep\relax}}
\toprule
...
\emph{Note on mode 6.} \texttt{README.\allowbreak{}md} \S\kern0.13em \ref{sec:5} labels mode 6 ``repository migrated across organisations''. That framing was \textbf{superseded on 2026-07-25} once id$\rightarrow$key stability was measured (\texttt{PROJECT\_\allowbreak{}STATE.\allowbreak{}md} \S\kern0.13em \ref{sec:2}, \texttt{5179907}). The current statement is below, and it is the one this paper uses.
...
\centering
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.2648\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.3608\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.3744\linewidth-2\tabcolsep\relax}}
\toprule
...
\textbf{``345 of 349 episodes traceable to 323 tickets'' --- the unit matters.} An estimate is a property of a ticket, not an episode; several episodes share a ticket. An earlier draft used 0/345 where 0/323 is correct (\texttt{PROJECT\_\allowbreak{}STATE.\allowbreak{}md} \S\kern0.13em \ref{sec:7}).

\textbf{Two unrelated quantities are both 92.3\% and are labelled apart.} 92.3\%-A is traceability (26,125 / 28,290 Hadoop commits citing a key, four-key probe); 92.3\%-B is RefactoringMiner chunk coverage (3,175 / 3,440). Different numerators, denominators, scopes and claims. This paper uses \textbf{92.3\%-A only} (\texttt{paper/\allowbreak{}numbers.\allowbreak{}md} \S\kern0.13em \ref{sec:2}).
...
\centering
\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.3889\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2121\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2072\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.1918\linewidth-2\tabcolsep\relax}}
\toprule
...
\textbf{Five is also exactly the count that maximises the ceiling over every possible count}: four gives 0.765, six gives 0.837, seven gives 0.833. The six were selected by a stated semantic criterion, they are individually defensible, and the finding is robust at 0.833--0.840 across plus or minus two comments --- but the criterion was applied by the same party that reported the resulting number, and it landed on the global maximum. Stated here rather than left for a reader to find (\texttt{audit/\allowbreak{}JUDGMENTS.\allowbreak{}md} \S\kern0.13em \ref{sec:3}).
...
\textdagger{} held-out corpus: coverage and existence counts only, no outcome ever observed (\texttt{PROJECT\_\allowbreak{}STATE.\allowbreak{}md} \S\kern0.13em \ref{sec:3}). \textdaggerdbl{} quarantined 2026-07-30, dropped from the held-out corpus; coverage retained, outcome inputs sequestered in \texttt{spent/\allowbreak{}} (\texttt{spent/\allowbreak{}README.\allowbreak{}md}).
...
\textbf{No timing column exists in this table and none may be added for a \textdagger{} row.} Every quantity above is a coverage or existence count, which is what the held-out rule permits (\texttt{PROJECT\_\allowbreak{}STATE.\allowbreak{}md} \S\kern0.13em \ref{sec:3}).
...
\begin{tabular}{>{\raggedright\arraybackslash}p{\dimexpr 0.3070\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.3822\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.3108\linewidth-2\tabcolsep\relax}}
